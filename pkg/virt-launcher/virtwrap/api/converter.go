...
func Convert_v1_IgnitionData_To_api_Disk(disk *Disk, c *ConverterContext) error {
	disk.Source.File = fmt.Sprintf("%s/%s", ignition.GetDomainBasePath(c.VirtualMachine.Name, c.VirtualMachine.Namespace), c.VirtualMachine.Spec.Ignition.Data)
	disk.Type = "file"
	disk.Driver.Type = "raw"
	return nil
}
...
func Convert_v1_VirtualMachine_To_api_Domain(vmi *v1.VirtualMachineInstance, domain *Domain, c *ConverterContext) (err error) {
	...
	if vmi.Spec.Domain.Ignition != nil && vmi.Spec.Domain.Ignition.Data != "" {
		if domain.Spec.QEMUCmd == nil {
			domain.Spec.QEMUCmd = &Commandline{}
		}
