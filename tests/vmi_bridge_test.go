...
var _ = Describe("Bridge", func() {
	...
	tests.BeforeAll(func() {
		...
		runNetworkBridgeDevicePlugin := func() {
			...
			tests.RunCommand("kubectl", "adm", "policy", "add-scc-to-user", "privileged", "-n",
	...
	Context("Exposing multiple networks to the VM via bridge device plugin", func() {
		var vmi *v1.VirtualMachineInstance
	...
	Context("Exposing multiple interfaces for the same network to the VM via bridge device plugin", func() {
		var vmi *v1.VirtualMachineInstance
		...
		tests.BeforeAll(func() {
			...
			vmi.Spec.Networks = append(vmi.Spec.Networks,
				v1.Network{Name: networkName + "2",
					NetworkSource: v1.NetworkSource{
						Resource: &v1.ResourceNetwork{ResourceName: fmt.Sprintf("bridge.network.kubevirt.io/%s", networkName)}}})
